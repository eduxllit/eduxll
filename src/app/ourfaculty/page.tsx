import Image from "next/image";
import HeaderLayout from "../components/headerLayout/HeaderLayout";

const ManagementTeam = () => {
  return (
    <>
      <HeaderLayout>
        <div className="bg-[url('https://eeskxyo993o.exactdn.com/wp-content/uploads/2022/04/06.png?strip=all&lossy=1&sharp=1&ssl=1')] bg-cover bg-no-repeat w-full">
          <div className="bg-gradient-to-r from-[#052044] to-transparent">
            <div className="text-white font-semibold lg:h-[300px] sm:h-[220px] h-[160px] grid content-center items-center sm:px-32 px-8">
              <h2 className="sm:text-4xl text-2xl block">Our Faculty</h2>
            </div>
          </div>
        </div>

        <section className="bg-[#fff] py-[50px]">
          <div className="md:max-w-[1200px] w-[100%] mx-auto px-[12px]">
            <div className="w-[100%] text-center">
              <h2 className="sm:text-[35px] text-2xl text-black leading-[1.3] font-bold mb-[20px]">
              Our Faculty (full-time)
              </h2>
            </div>

            

<div className="grid grid-cols-2 max-sm:grid-cols-1 gap-[20px] pt-[30px]">
<div className="cardlist grid grid-cols-2   max-sm:grid-cols-1 gap-[20px]">
  <div className="imagecard">
    <Image src="/managementteam/1.webp" width={300} height={300} alt={""} objectFit="cover" className="w-[100%]" />
  </div>
  <div className="contentcard">
    <h3 className="text-[#da202f] text-xl font-bold mb-[10px] ">Name: Dr. KR Balachandran</h3>
    <p className="text-[14px] font-[500] leading-[1.3] text-[#21225F] mb-[8px]  ">Qualification:  Ph.D.(Operations Research) – University of California,Berkeley</p>
    <p className="text-[14px] font-[500] leading-[1.3] text-[#21225F] mb-[8px] ">Total Years of Work Experience: 52 years</p>
    <p className="text-[14px] font-[500] leading-[1.3] text-[#21225F] ">Expertise: Management Accounting, Operations, Control Systems</p>
  </div>
</div>


<div className="cardlist grid grid-cols-2 max-sm:grid-cols-1 gap-[20px]">
  <div className="imagecard">
    <Image src="/managementteam/5.png" width={300} height={300} alt={""} objectFit="cover" className="w-[100%]" />
  </div>
  <div className="contentcard">
    <h3 className="text-[#da202f] text-xl font-bold mb-[10px] ">Name: Dr. Ravikesh Srivastava</h3>
    <p className="text-[14px] font-[500] leading-[1.3]  text-[#21225F] mb-[8px]  ">Qualification: Ph.D.– Banaras Hindu University</p>
    <p className="text-[14px] font-[500] leading-[1.3] text-[#21225F] mb-[8px] ">Total Years of Work Experience: 30+ years</p>
    <p className="text-[14px] font-[500] leading-[1.3] text-[#21225F] ">Dr. Srivastava has served as the
Pro Vice chancellor of IMS
Unison University Dehradun.
He has earlier served in senior
academic and administrative
positions at IMT Ghaziabad,
FORE School of Management,
IIM Lucknow, SP Jain and other
reputed B-Schools. </p>
  </div>
</div>

<div className="cardlist grid grid-cols-2 max-sm:grid-cols-1 gap-[20px]">
  <div className="imagecard">
    <Image src="/managementteam/6.webp" width={300} height={300} alt={""} objectFit="cover" className="w-[100%]" />
  </div>
  <div className="contentcard">
    <h3 className="text-[#da202f] text-xl font-bold mb-[10px] ">Name: Dr. Rakhi Singh </h3>
    <p className="text-[14px] font-[500] leading-[1.3]  text-[#21225F] mb-[8px]  ">Qualification: Ph.D.
(Economics) – Banasthali
Vidyapith</p>
    <p className="text-[14px] font-[500] leading-[1.3] text-[#21225F] mb-[8px] ">Total Years of Work
    Experience: 27 Years</p>
    <p className="text-[14px] font-[500] leading-[1.3] text-[#21225F] ">Expertise: Economics,
Entrepreneurship,
Innovation, Family Business
Management, Small Business
& SME sector
 </p>
  </div>
</div>


<div className="cardlist grid grid-cols-2 max-sm:grid-cols-1 gap-[20px]">
  <div className="imagecard">
    <Image src="/managementteam/7.jpg" width={300} height={300} alt={""} objectFit="cover" className="w-[100%]" />
  </div>
  <div className="contentcard">
    <h3 className="text-[#da202f] text-xl font-bold mb-[10px] ">Name: Prof. Anand Khanna</h3>
    <p className="text-[14px] font-[500] leading-[1.3]  text-[#21225F] mb-[8px]  ">Anand Khanna has nearly four
decades of experience in
Industry and Academics.</p>
    <p className="text-[14px] font-[500] leading-[1.3] text-[#21225F] mb-[8px] ">His academic experience
covers teaching and research
at XLRI
Jamshedpur from 1978 to
1982, and at IMT Ghaziabad
from 2004 to 2022.</p>
    <p className="text-[14px] font-[500] leading-[1.3] text-[#21225F] ">He has held senior level
positions in Marketing and
General Management in
Mekaster Group, MDS
Switchgears Ltd
</p>
  </div>
</div>



<div className="cardlist grid grid-cols-2 max-sm:grid-cols-1 gap-[20px]">
  <div className="imagecard">
    <Image src="/managementteam/8.png" width={300} height={300} alt={""}  objectFit="cover" className="w-[100%]" />
  </div>
  <div className="contentcard">
    <h3 className="text-[#da202f] text-xl font-bold mb-[10px] ">Name: Dr. Zeeshan</h3>
    <p className="text-[14px] font-[500] leading-[1.3]  text-[#21225F] mb-[8px]  ">Qualification: Ph.D.
    (Economics)- BITS Pilani</p>
    <p className="text-[14px] font-[500] leading-[1.3] text-[#21225F] mb-[8px] ">Total Years of Work
    Experience: 06 Years</p>
    <p className="text-[14px] font-[500] leading-[1.3] text-[#21225F] ">Expertise: Applied Economics,
Research Methodology, Data
Analysis, Managerial
Economics, and
Entrepreneurship.
</p>
  </div>
</div>



</div>


          </div>
        </section>
      </HeaderLayout>
    </>
  );
}
export default ManagementTeam;




